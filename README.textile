...
All elements have some kind of structure that they must follow. For instance, in HTML5 it is illegal for an @<input />@ to appear directly inside of a @<body />@ tag (it must be inside a form). XHP allows you to define a content model which documentsmust adhere to. This is done with t he @children@ keyword, which uses a syntax similar to regular expressions. Note, that unlike @attribute@, @children@ may only appear once inside any class.
